...
int main() {
    ...
    char *pesan = strtok(inp, ";");
    char *jumlah = strtok(NULL, ";");

    if (pesan == NULL || jumlah == NULL) {
        printf("Salah format woy\n");
        return 1;
    }

    FILE *logFile = fopen("system.log", "a");
    if (logFile == NULL) {
        perror("Gagal buka file log");
        return 1;
    }

    fprintf(logFile, "Message from client: %s\n", pesan);
    fprintf(logFile, "Message count: %s\n", jumlah);

    fclose(logFile);
    printf("done system bruh\n");

    key_t key =  37;
    int shared = shmget(key, 1024, 0666 | IPC_CREAT);  
    char *shm_ptr = (char *)shmat(shared, NULL, 0);
    snprintf(shm_ptr, 1024, "Message: %s, Count: %s", pesan, jumlah);
